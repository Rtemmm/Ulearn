...
namespace Manipulation
{
	public static class VisualizerTask
	{
		...
		{
			var coordinates = ConvertWindowToMath(new PointF(e.X, e.Y), GetShoulderPos(form));
		...
		public static void DrawManipulator(Graphics graphics, PointF shoulderPos)
		{
			...
			var radius = 5;
			for (var i = 0; i < 3; i++)
				graphics.FillEllipse(JointBrush, joints[i].X - radius, joints[i].Y - radius, radius*2, radius*2);
